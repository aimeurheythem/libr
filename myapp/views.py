...
def update_student(request, pk):
    ...
    student.class_name = request.data["class_num"]
...
def rentBook(request):
    ...
    if serializer.is_valid():
        ...
        return Response({"Rentbook": result.data})
    else:
        return Response(serializer.errors)
...
def update_rentedBooks(request, pk):
    ...
    rent.book_id = get_object_or_404(Book, id=request.data["book_id"]) 
    rent.student_id = get_object_or_404(Student, id=request.data["student_id"])
    rent.rent_date = request.data["rent_date"]
    rent.return_date = request.data["return_date"]
    rent.isReturned = request.data["isReturned"]
...
def libraryCard(request):
    ...
    if serializer.is_valid():
        ...
        return Response({"LibraryCard": result.data})
    else:
        return Response(serializer.errors)
